...
import { FormAddLabPasien } from '../containers/FormAddLabPasien';
import { Appointment } from '../utils/Appointment';

export class AddLabPasien extends React.Component {
	/** 
	...
	handleFormSubmit(e) {
		...
		Appointment.addLabPasien(dataJson).then(response => {
			if (response.status === 200) {
				this.setState({
					loading: false,
				})
				alert(`Sukses add lab pasien ${this.state.pasien.nama}`)
			} else {
				this.setState({
					loading: false
				})
				alert(`Gagal add lab pasien ${this.state.pasien.nama}`)
			}
		})
	}
	...
	render() {
		if (this.state.loading) {
			...
			return (
				<FormAddLabPasien pasien={this.state.pasien} onSubmit={this.handleFormSubmit} />
			)
		}
	}
}